...
#GPIO.setup(3, GPIO.IN)#se refiere al num de pin 3 corresponde al gpio 2
#GPIO.setup(11, GPIO.OUT)#igual es el mum de pin 11 corresponde al GPIO 17
...
out1 = 17                       # en las variables out1, out2 
...
class FormularioOperacion:
    def __init__(self):
        ...
        self.ventana1=tk.Tk()
        self.ventana1.title("ESTACIONAMIENTO CON BARRERAS")
...
# -----------------------------------
    def ExpedirRfid(self):    
        ...
        self.mi_Label = ttk.Label(self.pagina1, text="Yo soy un Label") #Creación del Label
        self.mi_Label.config(width =17)
        self.mi_Label.config(background="white") #Cambiar color de fondo
        self.mi_Label.config(font=('Arial', 62)) #Cambiar tipo y tamaño de fuente
        self.mi_Label.grid(column=0, row=8, padx=0, pady=0)
        self.Reloj = ttk.Label(self.pagina1, text="Hora y fecha", width = 17, background = '#FD6')
        #self.Reloj.config(font=('Arial', 62)
        self.Reloj.grid(column=0, row=16, padx=0, pady=0)
...
#         if io.input(in1):
#                 self.presenciaAuto.config(background = '#BBB')        # si la entrada 1 es a 1 enciende la label
#                 io.output(out1,0)
#                 #print("entro a check imputs y esta desconectado el boton")
#         else:
#                 self.presenciaAuto.config(background = '#FD4')        # si no lo es la apaga
#                 io.output(out1,1)
#                 self.ManejoEntradas()
#         self.ventana1.after(50, self.check_inputs)          # activa un timer de 50mSeg.
       #llamar a operacion y meter en Entrada la hora en el momento actual y un numero de corte 0
    def check_inputs(self):
        if io.input(in1):
                self.loopDet.config(text = "no siente auto")
                #self.presenciaAuto.config(text = "unpress")        # si la entrada 1 es a 1 enciende la label
                io.output(out1,0)
                #print("entro a check imputs y esta desconectado el boton")
        else:
                print("QUE PASA AQUI")
                self.loopDet.config(text = "SI HAY")
                #self.presenciaAuto.config(text = "press")        # si no lo es la apaga
                
                io.output(out1,1)
                if __name__ == '__main__':
                        RecibeRasPi = serial.Serial('/dev/ttyACM0', 9600, timeout=1)
                        EnviaRasPi=serial.Serial('/dev/ttyACM0',baudrate=9600, timeout = 3.0)
                        RecibeRasPi.flush()
                        print("poner banderas")
                        flag = True
                        band1 = True
                        band2 = True
                        band3 = True
                        ...
                        while flag == True:
                                ...
                                if band1 == True:   
                                   comando = "A"     
                                   EnviaRasPi.write(comando.encode())
                                   print ('lo que envia raspi es ', comando)
                                   band1 = False 
                                   band2 = False
                                   time.sleep(.500)        
                                if band2 == False:
                                   comando = "r"
                                   EnviaRasPi.write(comando.encode())
                                   print ('lo que envia raspi desde aqui es ', comando)                                                                
                                   band1 = True                          
                                   band2 = True
                                   time.sleep(.500)
                                if band3 == False: 
                                   comando = "E"
                                   EnviaRasPi.write(comando.encode())
                                   print ('lo que envia para finalizar', comando)                                
                                   time.sleep(.150)          
                                if RecibeRasPi.in_waiting > 0: 
                                        contador = 1+contador 
                                        print(contador)  
                                        line = RecibeRasPi.readline().decode('utf-8').rstrip()
                                        print('lo que recibe RasPi',line)
                                        print ("mide",len(line))
                                        if line == "press":
                                                print ("recibio press, pone banderas para que mande E y finalice")
                                                print(line)
                                                band1 = False
                                                band2 = True
                                                band3 = False
                                                ...
                                                now = datetime.now()
                                                self.presenciaAuto.config(text = now)
                                                self.RFID.set("")
                                        if len(line) >= 8:
                                                print("se recibio un pensionado igual E para mandar finalizar")
                                                print(line)
                                                band1 = False
                                                band2 = True
                                                band3 = False
                                                self.RFID.set(line)
                                                self.presenciaAuto.config(text = "")
                                                fechaPrueba = datetime.today()
                                                datos= (line, fechaPrueba,) 
                                                self.operacion1.RFID(datos) 
                                        if line == "E":
                                                print("Raspi mando la E y se sale de preguntar poniendo flag en false para salir del ciclo")
                                                print(line)
                                                flag = False
                                        line = ''  
                                        if contador == 10:
